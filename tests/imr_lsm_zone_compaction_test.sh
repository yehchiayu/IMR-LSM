...
main()
{
    ...
    dd if=/dev/zero of="${DEVICE}" bs="${BLOCK_SIZE}" seek="${zone_start}" \
        count="${TOTAL_ITEMS}" conv=notrunc oflag=direct status=progress
    sync
